...
class MemberForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'date_birth': forms.DateInput(attrs={'class': 'form-control-sm', 'type': 'date'}),
            'city_birth': forms.Select(attrs={'class': 'form-select'}),
            'state_birth': forms.Select(attrs={'class': 'form-select'}),
            'date_baptism': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}),
            'address': forms.TextInput(attrs={'class': 'form-control'}),
            'cep': forms.TextInput(attrs={'class': 'form-control', 'pattern': '[0-9]{5}-[0-9]{3 }', 'placeholder': '00000-000' })
        }
        labels = {
            'name': 'NOME',
            'cpf': 'CPF',
